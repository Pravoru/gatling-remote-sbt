...
if exist "%GATLING_HOME%\bin\run.bat" goto gotHome
rem if gatling home is not correctly set
if not "%GATLING_HOME%" == "" goto badHome
rem if not try current folder
if exist "%OLD_DIR%\bin\run.bat" set "GATLING_HOME=%OLD_DIR%" && goto gotHome
rem if not try parent folder
if exist "%DEFAULT_GATLING_HOME%\bin\run.bat" set "GATLING_HOME=%DEFAULT_GATLING_HOME%" && goto gotHome
...
rem se t COMPILATION_CLASSPATH=""
rem for %%i in ("%GATLING_HOME%\lib\*.jar") do call :addToPath "%%i"
@REM java %COMPILER_OPTS% -cp %COMPILER_CLASSPATH% io.gatling.compiler.ZincCompiler -ccp %COMPILATION_CLASSPATH% %USER_ARGS%  2>NUL
rem Run Gatling
java %JAVA_OPTS% -Dgatling.data.graphite.rootPathPrefix=grafiteRootPathPrefix.%COMPUTERNAME% -cp %GATLING_CLASSPATH% io.gatling.app.Gatling %USER_ARGS%
